...
from .api_model_converter import ApiModelConverter
from .api_model_property_converter import ApiModelPropertyConverter
from .api_operation_converter import ApiOperationConverter
    ...
    def process_task(self, path):
        ...
        for i, fp in enumerate(java_files, start=1):
            ...
            if self.var_api_model_property.get():
                content, added1 = ApiModelPropertyConverter.convert(content)
                if added1:
                    modified = True
                    self.log(f"  ApiModelProperty → JavaDoc 生成 {added1} 条")

            if self.var_api_operation.get():
                content, added2 = ApiOperationConverter.convert(content)
                if added2:
                    modified = True
                    self.log(f"  ApiOperation → JavaDoc 生成 {added2} 条")

            if self.var_api_model.get():
                content, added1 = ApiModelConverter.convert(content)
                if added1:
                    modified = True
                    self.log(f"  ApiModel → JavaDoc 生成 {added1} 条")

            if self.var_api.get():
                content, added1 = ApiConverter.convert(content)
                if added1:
                    modified = True
                    self.log(f"  ApiModel → JavaDoc 生成 {added1} 条")
